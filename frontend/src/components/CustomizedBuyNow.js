...
const CustomizedBuyNowModal = ({ isOpen, onClose, totalPrice, items }) => {
  ...
  const onSubmit = (data) => {
    const orderData = {
      customItems: items.map((el) => [{ item: el._id }]), // Assuming buying a single item
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center mb-4">
            <img
              src={items[0].customizable_menu_id.image_url}
              alt={items[0].customizable_menu_id.name}
              className="w-16 h-16 object-cover rounded-md mr-4"
            />
            <div>
              <h4 className="text-lg font-semibold">{items[0].customizable_menu_id.name}</h4>
              <p className="text-gray-700">{items.map(el=>el.name).toString()}</p>
...
export default CustomizedBuyNowModal;
